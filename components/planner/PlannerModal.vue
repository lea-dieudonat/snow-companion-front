...
<template>
  ...
  <!-- Modal full-screen (toujours dans le DOM pour préserver l'état) -->
  <div
    class="fixed inset-0 z-300 flex flex-col bg-snow-50 dark:bg-mountain-900 transition-transform duration-300 ease-in-out"
    :class="isOpen ? 'translate-y-0' : 'translate-y-full'"
    :aria-hidden="!isOpen"
  >
    <!-- En-tête ────────────────────────────────────────────────────────────── -->
    <div class="flex items-center justify-between px-4 py-3 border-b border-snow-200 dark:border-mountain-700 shrink-0">
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-xl bg-ice-100 dark:bg-ice-900/40 flex items-center justify-center">
          <UIcon name="i-lucide-sparkles" class="text-lg text-ice-500" />
        </div>
        <h1 class="text-base font-semibold text-mountain-900 dark:text-snow-50">Planner IA</h1>
      </div>
      <div class="flex items-center gap-2">
        <UButton
    ...
    <!-- Corps ──────────────────────────────────────────────────────────────── -->
    <div class="flex-1 flex flex-col min-h-0 px-4 pt-4 pb-4">
      <!-- Bannière onboarding ─────────────────────────────────────────────── -->
      <UAlert
      ...
      >
        <template #actions>
          <UButton color="warning" variant="soft" size="xs" @click="emit('close')">
      ...
      <!-- Zone messages ───────────────────────────────────────────────────── -->
      <div ref="messagesEl" class="flex-1 overflow-y-auto space-y-4 pb-2 pr-1">
