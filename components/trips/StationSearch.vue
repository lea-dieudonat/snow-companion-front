  ...
  <UCard class="overflow-hidden">
    ...
    <div class="space-y-6">
      ...
      <UFormField label="Station">
        <UInput
v-model="searchQuery" icon="i-lucide-mountain-snow" placeholder="Val Thorens, Les 2 Alpes..." size="lg"
          class="w-full" />
      </UFormField>
      ...
      <!-- Max distance -->
      <AppRangeFilter
v-model="filters.maxDistance" icon="i-lucide-map-pin"
        label="Distance max" :min="50" :max="500" :step="50"
        value-suffix=" km" min-label="50 km" max-label="500 km" />

      <!-- Lift pass price -->
      <AppRangeFilter
v-model="filters.maxLiftPassPrice" icon="i-lucide-ticket"
        label="Forfait max" :min="30" :max="80" :step="5"
        value-suffix="€/jour" min-label="30€" max-label="80€" />

      <!-- Lodging price -->
      <AppRangeFilter
v-model="filters.maxLodgingPrice" icon="i-lucide-bed"
...
v-for="option in levelOptions" :key="option.value"
    ...
    <template #footer>
      <UButton
block size="lg" color="neutral" variant="solid" leading-icon="i-lucide-search" class="relative overflow-hidden group
        ...
        <span
class="absolute inset-y-0 left-1/2 -translate-x-1/2 w-24
