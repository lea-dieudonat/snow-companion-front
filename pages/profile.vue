...
onMounted(async () => {
  await Promise.all([sessionsStore.load(), favoritesStore.load()]);
});
...
<template>
  <div class="page-container-sm">
    ...
    <UCard>
      ...
      <div class="space-y-2">
        <UButton color="neutral" variant="ghost" block icon="i-lucide-bar-chart-3" trailing-icon="i-lucide-arrow-right"
          to="/tracking" class="justify-between">
          Voir mes sessions
        </UButton>
        <UButton color="neutral" variant="ghost" block icon="i-lucide-map" trailing-icon="i-lucide-arrow-right"
          to="/trips" class="justify-between">
          Voir mes trips
        </UButton>
        <UDivider class="my-2" />
